"""
科大讯飞AST客户端 - 音频流式识别
支持实时音频流传输和语音识别结果接收
"""
import asyncio
import websockets
import base64
import json
import uuid
import ssl
from dotenv import load_dotenv

# 加载环境变量
load_dotenv()

# 移除代理设置，否则websocket链接报错
import os
os.environ.pop('http_proxy', None)
os.environ.pop('https_proxy', None)
os.environ.pop('HTTP_PROXY', None)
os.environ.pop('HTTPS_PROXY', None)


# 配置参数
WS_URL = "wss://182.150.59.81:31843/ai/agent/v1/ast"  # WebSocket服务地址
token = os.getenv("TOKEN")  # 从环境变量读取认证令牌
if not token:
    raise ValueError("TOKEN 未设置，请在 .env 文件中配置 TOKEN")
AUDIO_PATH = "data/近远场测试.wav"  # 音频文件路径
FRAME_SIZE = 4096  # 音频帧大小（字节）
# 时长(秒) = FRAME_SIZE / (采样率 × 位深度/8)
# 时长(秒) = 4096 / (16000 × 16/8)
# 时长(秒) = 4096 / (16000 × 2)
# 时长(秒) = 4096 / 32000
# 时长(秒) = 0.128 秒 = 128 毫秒
INTERVAL = 0.04  # 发送间隔（秒）
OUTPUT_FILE = "recognition_results.json"  # 输出文件路径

def gen_trace_id():
    """生成唯一的追踪ID"""
    return str(uuid.uuid4())

async def send_audio(ws, audio_path):
    """
    发送音频数据到WebSocket服务器
    
    Args:
        ws: WebSocket连接对象
        audio_path: 音频文件路径
    """
    trace_id = gen_trace_id()  # 生成追踪ID
    biz_id = "test_bizid_001"  # 业务ID
    app_id = "123456"  # 应用ID
    status = 0  # 状态：0-开始，1-中间，2-结束

    with open(audio_path, "rb") as f:
        while True:
            data = f.read(FRAME_SIZE)  # 读取音频数据块
            
            # 如果读取的数据小于请求的大小，说明这是最后一块数据
            is_last_chunk = len(data) < FRAME_SIZE

            if is_last_chunk:
                status = 2  # 设置为结束状态
            
            audio_b64 = base64.b64encode(data).decode()  # 将音频数据编码为Base64
            
            # 构建音频载荷
            payload = {
                "audio": {
                    "audio": audio_b64
                }
            }
            
            # 只在第一块数据时添加热词（用于提高识别准确率）
            if status == 0:
                payload["text"] = {
                    "text": "张三疯|向钱看"  # 热词配置
                }
            
            # 构建完整的消息结构
            msg = {
                "header": {
                    "traceId": trace_id,  # 追踪ID
                    "appId": app_id,  # 应用ID
                    "bizId": biz_id,  # 业务ID
                    "status": status,  # 状态码
                    "resIdList": []  # 资源ID列表
                },
                "parameter": {
                    "engine": {
                        "wdec_param_LanguageTypeChoice": "5"  # 语言类型选择
                    }
                },
                "payload": payload  # 载荷数据
            }
            await ws.send(json.dumps(msg))  # 发送JSON消息
        
            
            # 如果是最后一块数据，发送后立即退出循环
            if is_last_chunk or status == 2:
                break
                
            await asyncio.sleep(INTERVAL)  # 等待指定间隔
            status = 1  # 仅第一帧 status=0，后续为1

async def receive_result(ws):
    """
    接收并处理WebSocket服务器返回的识别结果
    
    Args:
        ws: WebSocket连接对象
    """
    results = []  # 存储所有结果
    sequence_number = 1  # 序列号
    accumulated_text = ""  # 累积的文本内容
    role = "(角色未知)"  # 角色标识
    
    async for message in ws:
        try:
            resp = json.loads(message)  # 解析JSON消息
            
            # 添加序号和时间戳到结果中
            result_with_sequence = {
                "sequence": sequence_number,
                "timestamp": asyncio.get_event_loop().time(),
                "data": resp
            }
            
            results.append(result_with_sequence)
            
            # 解析识别结果
            if "payload" in resp and "result" in resp["payload"]:
                result = resp["payload"]["result"]
                bg = result.get("bg")  # 开始时间
                ed = result.get("ed")  # 结束时间
                msgtype = result.get("msgtype", "")  # 消息类型
               
                
                # 提取当前帧的文本内容
                current_text = ""
                if "ws" in result:  # 词序列
                    for ws_item in result["ws"]:
                        if "cw" in ws_item:  # 词
                            for cw_item in ws_item["cw"]:
                                if "w" in cw_item:  # 词内容
                                    rl = cw_item.get("rl", "")  # 角色ID
                                    if rl != 0:
                                        role = f"(角色{rl})"
                                    current_text += (cw_item["w"] + role)
                                    
                                  
                current_text += f"[{bg}-{ed}]"  # 添加时间戳信息
                
                # 根据消息类型处理累积文本
                if msgtype == "progressive":
                    # 中间状态：显示当前累积的文本
                    status_label = "【中间状态】"
                    display_text = accumulated_text + current_text
                elif msgtype == "sentence":
                    # 最终状态：将当前文本添加到累积文本中
                    accumulated_text += current_text
                    status_label = "【最终状态】"
                    display_text = accumulated_text
                else:
                    status_label = "【未知状态】"
                    display_text = accumulated_text + current_text
                
                # 打印累积的文本内容
                if current_text or accumulated_text:
                    print(f"收到结果序号#{sequence_number} {status_label}: {display_text}")
                else:
                    print(f"收到结果序号#{sequence_number}: 无文本内容")
                 # 可以根据 resp["header"]["status"] == 2 判断是否结束
            if resp.get("header", {}).get("status") == 2:
                print("识别结束")
                
                # 保存所有结果到JSON文件
                with open(OUTPUT_FILE, 'w', encoding='utf-8') as f:
                    json.dump(results, f, ensure_ascii=False, indent=2)
                
                print(f"所有识别结果已保存到 {OUTPUT_FILE}")
                print(f"最终累积文本: {accumulated_text}")
                break
                
            sequence_number += 1  # 递增序列号
            
        except Exception as e:
            print("解析服务端消息失败:", e)

async def main():
    """
    主函数：建立WebSocket连接并启动音频发送和结果接收任务
    """
    headers = {
        'Authorization': f'Bearer {token}'  # 设置认证头
    }
    
    # 忽略 SSL 校验（仅用于测试环境）
    ssl_context = ssl.create_default_context()
    ssl_context.check_hostname = False
    ssl_context.verify_mode = ssl.CERT_NONE
    
    # 建立WebSocket连接并并发执行发送和接收任务
    async with websockets.connect(WS_URL, additional_headers=headers, ssl=ssl_context) as ws:
        send_task = asyncio.create_task(send_audio(ws, AUDIO_PATH))  # 创建音频发送任务
        recv_task = asyncio.create_task(receive_result(ws))  # 创建结果接收任务
        await asyncio.gather(send_task, recv_task)  # 并发执行两个任务

if __name__ == "__main__":
    """程序入口点"""
    asyncio.run(main())